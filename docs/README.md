...
- **Week 0** *（对应校历 2025-2026 学年第一学期第 12 周）*
    - 周一（12.1）：发布项目和 [业务要求文档](业务要求.md)，开始完成**需求分析文档**；
    - 周六（12.6）：检查**需求分析文档**；
    - 周日（12.7）：发布[标准要求文档](标准要求.md)，开始完成**总体设计文档**；
- **Week 1** *（对应校历 2025-2026 学年第一学期第 13 周）*
    - 周五（12.12）：**中期检查**，检查总体设计文档编写情况以及主体部分构思；
- **Week 2**
    - 周一（12.15 18:30 前）：**通过**前置作业评测；
- **Week 4** *（对应校历第 16 周）*
    - 周一（12.29 18:30 前）：**通过**测试点评测；
...
- Commit 频率不可以太低（**要求整个项目 commit 次数 > 5**）；
...
- Commit message 内容可以清晰简要但不能缺失、不能是无意义内容。
    ...
    - 合理的 message 示例：
        ...
        - feat: partial implementation of unrolled linked list
    - 不合理的 message 示例：
        - update
        - Add files via upload
        - Update README.md
        - bugfix *（具体修了什么？）*
        - 20251212 *（git 会自动存储 commit 时间，不用在 commit message 里说明。）*
